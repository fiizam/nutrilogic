...
import { Activity, Plus, TrendingDown, Calendar, Scale, Award, Trash2, Utensils, Flame, Info } from "lucide-react";
  ...
  const handleAddProgress = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/progress", {
        ...
        body: JSON.stringify({ berat: newBerat })
      });
      if (res.ok) {
        setNewBerat("");
        fetchData();
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm font-semibold text-slate-600">Hi, {session.user?.name}</span>
            <Link href="/planner" className="text-xs font-bold uppercase tracking-widest text-emerald-600 bg-emerald-50 border border-emerald-100 px-4 py-2 rounded-xl hover:bg-emerald-100 transition-colors">
              Buat Plan Baru
            </Link>
          </div>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-6 sm:px-8 pt-10">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Profil Pengguna & Progress</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl flex flex-col items-center justify-center text-center relative overflow-hidden">
             <div className="w-12 h-12 bg-emerald-500/20 rounded-2xl flex items-center justify-center mb-3 border border-emerald-500/30 relative z-10">
              <Award className="w-6 h-6 text-emerald-400"/>
            </div>
            <p className="text-[10px] text-emerald-400 uppercase font-bold tracking-widest mb-1 relative z-10">Plan Aktif</p>
            <p className="text-2xl font-black text-white relative z-10">{latestPlan ? `${latestPlan.energi.tdeeTarget} kcal` : 'Belum Ada'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 h-fit">
            <h3 className="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2 mb-6">
              <Calendar className="w-5 h-5 text-indigo-500" /> Log Berat Badan
            </h3>
            
            <form onSubmit={handleAddProgress} className="flex gap-3 mb-8">
              <div className="flex-1 relative">
                <input 
                  type="number" 
                  step="0.1"
                  required
                  placeholder="Masukkan berat badan baru" 
                  value={newBerat} 
                  onChange={(e) => setNewBerat(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-emerald-500" 
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">KG</span>
              </div>
              <button disabled={isSubmitting} type="submit" className="bg-slate-900 text-white px-5 rounded-xl font-semibold hover:bg-emerald-600 transition-colors disabled:opacity-50 flex items-center justify-center">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {progress.length === 0 ? (
                <p className="text-sm text-slate-500 text-center py-4">Belum ada data. Mulai lacak berat badan Anda hari ini!</p>
              ) : (
                            ...
                            {data.olahraga.map((ol, idx) => (
                              <div key={idx} className="bg-white p-3 rounded-xl border border-slate-200">
                                <div className="flex justify-between items-center mb-1">
                                  <span className="text-xs font-bold text-indigo-600">{ol.nama}</span>
                                  <span className="text-[10px] font-bold bg-indigo-50 text-indigo-500 px-2 py-0.5 rounded uppercase tracking-wider">{ol.kategori}</span>
                                </div>
